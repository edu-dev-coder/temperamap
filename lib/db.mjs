...
export async function listTestSessions(status) {
  ...
  if (error) throw error;
  const sessions = mapRows(data);
  const missingInfo = sessions.filter(s => !s.userEmail && s.userId && s.userId !== "anonymous");
  if (missingInfo.length > 0) {
    const userIds = [...new Set(missingInfo.map(s => s.userId))];
    const { data: users } = await supabase.from("users").select("id, email, first_name, last_name").in("id", userIds);
    if (users) {
      const userMap = {};
      for (const u of users) userMap[u.id] = u;
      for (const s of missingInfo) {
        const u = userMap[s.userId];
        if (u) {
          s.userEmail = u.email || "";
          s.userName = `${u.first_name || ""} ${u.last_name || ""}`.trim();
        }
      }
    }
  }
